...
mod tests {
    ...
    fn test_best_hand() {
        assert_eq!(
            best_hand(vec![2, 3, 5, 7, 11], vec!['d', 'd', 'd', 'd', 'd']),
            "Flush".to_owned()
        );
        assert_eq!(
            best_hand(vec![2, 2, 2, 7, 11], vec!['d', 'a', 'c', 'b', 'd']),
            "Three of a Kind".to_owned()
        );
        assert_eq!(
            best_hand(vec![2, 3, 5, 7, 7], vec!['d', 'a', 'c', 'b', 'd']),
            "Pair".to_owned()
        );
        assert_eq!(
            best_hand(vec![2, 3, 5, 7, 10], vec!['d', 'a', 'c', 'b', 'd']),
            "High Card".to_owned()
        );
    }
}
